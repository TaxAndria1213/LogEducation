...
class FinanceRecouvrementService extends Service {
  ...
  async getPolicy() {
    return this.customRequest.get("/policy");
  }

  async savePolicy(payload: FinanceRecoveryPolicyPayload) {
    return this.customRequest.put("/policy", payload);
  }

  async approvePolicy() {
    return this.customRequest.post("/policy/approve", {});
  }

  async rejectPolicy(motif?: string | null) {
    return this.customRequest.post("/policy/reject", { motif: motif ?? null });
  }

  async createPaymentPromise(payload: Record<string, unknown>) {
    return this.customRequest.post("/payment-promises", payload);
  }

  async getPaymentPromises(params: Record<string, unknown> = {}) {
    return this.customRequest.get("/payment-promises", params);
  }

  async keepPaymentPromise(id: string) {
    return this.customRequest.post(`/payment-promises/${id}/keep`, {});
  }

  async breakPaymentPromise(id: string) {
    return this.customRequest.post(`/payment-promises/${id}/break`, {});
  }

  async cancelPaymentPromise(id: string) {
    return this.customRequest.post(`/payment-promises/${id}/cancel`, {});
  }

  async createAdministrativeRestriction(payload: Record<string, unknown>) {
    return this.customRequest.post("/administrative-restrictions", payload);
  }

  async getAdministrativeRestrictions(params: Record<string, unknown> = {}) {
    return this.customRequest.get("/administrative-restrictions", params);
  }

  async liftAdministrativeRestriction(id: string) {
    return this.customRequest.post(`/administrative-restrictions/${id}/lift`, {});
  }

  async createCollectionCase(payload: Record<string, unknown>) {
    return this.customRequest.post("/collection-cases", payload);
  }

  async getCollectionCases(params: Record<string, unknown> = {}) {
    return this.customRequest.get("/collection-cases", params);
  }

  async updateCollectionCaseStatus(id: string, payload: Record<string, unknown>) {
    return this.customRequest.post(`/collection-cases/${id}/status`, payload);
  }
}
