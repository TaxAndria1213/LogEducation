...
class CreneauHoraireApp {
    ...
    private async create(req: Request, res: R, next: NextFunction): Promise<void> {
        try {
            const data: CreneauHoraire = req.body;
            const result = await this.creneauHoraire.create(data);
            Response.success(res, "Creneau horaire created.", result);
        } catch (error) {
            Response.error(res, "Erreur lors de la creation du creneau horaire", 400, error as Error);
            next(error);
        }
    }
    ...
    private async getAll(req: Request, res: R, next: NextFunction): Promise<void> {
        try {
            ...
            Response.error(res, "Erreur lors de la recuperation des creneaux horaires", 400, error as Error);
            next(error);
        }
    }
